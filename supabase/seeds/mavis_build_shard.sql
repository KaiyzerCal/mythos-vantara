...
INSERT INTO mavis_notes (user_id, title, content, tags, aliases, properties)
SELECT
...
Run these five schedules in the SQL Editor with your real project ref and service role key.
Templates are in: supabase/migrations/20260516170000_mavis_sr_goals.sql

Schedules to configure:
- mavis-council-heartbeat: every 4 hours  →  0 */4 * * *
- mavis-daily-notes: end of day           →  55 23 * * *
- mavis-spaced-repetition: morning        →  0 8 * * *
- mavis-weekly-review: Sunday evening     →  0 22 * * 0
- mavis-monthly-review: last day          →  30 23 28-31 * *
...
The only remaining setup items are operator tasks: pg_cron scheduling above, and env vars in Lovable project settings. No more code is needed unless you want to extend into new domains.
$NOTE$,
  ARRAY['system', 'architecture', 'build-log', 'mavis', 'reference'],
  ARRAY['MAVIS Build Log', 'System Architecture', 'Session Log'],
  '{}'::jsonb
WHERE NOT EXISTS (
  SELECT 1 FROM mavis_notes
  WHERE title = 'MAVIS System Build — Complete Architecture Log'
