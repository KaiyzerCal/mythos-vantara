...
async function downloadFileContent(fileId: string): Promise<FileResult> {
  try {
    ...
    return {
      text: raw.slice(0, MAX) + (truncated ? "\n\n[...truncated — file exceeded 18,000 characters]" : ""),
    };
  } catch (err) {
...
serve(async (req) => {
  ...
  const process = async () => {
    ...
    const doc = (message.document ?? message.audio) as Record<string, unknown> | undefined;
    if (!wasVoice && !wasPhoto && doc?.file_id) {
      ...
      // Image sent as a document (e.g. uncompressed photo) → route to vision
      if (result.isImage) {
        const prompt   = text || `Analyze this image: ${fileName}`;
        const analysis = await analyzePhoto(String(doc.file_id), prompt, uid);
        await send(chatId, analysis ? `📸 ${analysis}` : "⚠️ Could not analyze image.");
        return;
      }

      // Text-based file — inject content into the MAVIS prompt
