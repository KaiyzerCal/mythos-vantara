...
async function generatePDF(
  ...
  // White field (entire page is already white in pdf-lib)
  // Top branding bar — narrow, light
  cover.drawRectangle({ x: 0, y: H - 44, width: W, height: 44, color: light });
  cover.drawRectangle({ x: 0, y: H - 47, width: W, height: 3, color: purple });
  cover.drawText("CODEXOS  ·  MAVIS AUTONOMOUS PRODUCT", { x: ML, y: H - 27, size: 7.5, font: regular, color: muted });
  const priceW2 = bold.widthOfTextAtSize(price, 11);
  // Price pill — outlined box top-right
  cover.drawRectangle({ x: W - MR - priceW2 - 12, y: H - 34, width: priceW2 + 12, height: 18, color: light });
  cover.drawLine({ start: { x: W - MR - priceW2 - 12, y: H - 16 }, end: { x: W - MR, y: H - 16 }, thickness: 0.6, color: purple, opacity: 0.5 });
  cover.drawLine({ start: { x: W - MR - priceW2 - 12, y: H - 34 }, end: { x: W - MR, y: H - 34 }, thickness: 0.6, color: purple, opacity: 0.5 });
  cover.drawLine({ start: { x: W - MR - priceW2 - 12, y: H - 34 }, end: { x: W - MR - priceW2 - 12, y: H - 16 }, thickness: 0.6, color: purple, opacity: 0.5 });
  cover.drawLine({ start: { x: W - MR, y: H - 34 }, end: { x: W - MR, y: H - 16 }, thickness: 0.6, color: purple, opacity: 0.5 });
  cover.drawText(price, { x: W - MR - priceW2 - 6, y: H - 29, size: 11, font: bold, color: purple });

  // Title block — large dark text, lots of air
  const titleLines = wrapText(title, bold, 30, contentW);
  let ty = H - 110;
  for (const line of titleLines) { cover.drawText(line, { x: ML, y: ty, size: 30, font: bold, color: dark }); ty -= 40; }

  // Category pill below title
  ty -= 6;
  const catW = bold.widthOfTextAtSize(cat, 8);
  cover.drawRectangle({ x: ML, y: ty - 2, width: catW + 16, height: 16, color: purple });
  cover.drawText(cat, { x: ML + 8, y: ty + 4, size: 8, font: bold, color: white });
  ty -= 24;

  // Thin purple rule
  ty -= 12;
  cover.drawLine({ start: { x: ML, y: ty }, end: { x: W - MR, y: ty }, thickness: 0.75, color: purple, opacity: 0.25 });
  ty -= 20;

  // Description — readable dark text
  if (description) {
    for (const line of wrapText(description, regular, 12, contentW).slice(0, 10)) {
      cover.drawText(line, { x: ML, y: ty, size: 12, font: regular, color: dark }); ty -= 18;
    }
  }

  // Bottom footer
  cover.drawRectangle({ x: 0, y: 0, width: W, height: 36, color: light });
  cover.drawRectangle({ x: 0, y: 36, width: W, height: 0.75, color: purple, opacity: 0.2 });
  cover.drawText("Generated by MAVIS  ·  Machine Autonomous Vantara Intelligence System  ·  CODEXOS", { x: ML, y: 13, size: 7.5, font: regular, color: muted });
  ...
  const drawPageChrome = (pg: ReturnType<typeof doc.addPage>, num: number) => {
    ...
    pg.drawRectangle({ x: 0, y: H - HEADER_H - 1, width: W, height: 1.5, color: purple, opacity: 0.18 });
    pg.drawText(title.slice(0, 55), { x: ML, y: H - 20, size: 7.5, font: regular, color: muted });
    const pn = `${num}`;
    const pnW = regular.widthOfTextAtSize(pn, 8);
    pg.drawCircle({ x: W - MR - pnW / 2 - 6, y: H - 17, size: 8, color: purple, opacity: 0.12 });
    pg.drawText(pn, { x: W - MR - pnW - 6, y: H - 20, size: 8, font: bold, color: muted });
    pg.drawLine({ start: { x: ML, y: MB }, end: { x: W - MR, y: MB }, thickness: 0.35, color: muted, opacity: 0.2 });
  };
  ...
  for (const block of blocks) {
    ...
    if (block.type === "h1" || block.type === "h2") {
      need(60); y -= 14;
      // Left accent bar + text — no background band
      const h2lines = wrapText(block.text, bold, 15, contentW - 14);
      const h2H = h2lines.length * 22 + 4;
      page.drawRectangle({ x: ML - 12, y: y - h2H + 18, width: 3, height: h2H, color: purple });
      for (const l of h2lines) drawLine(l, ML + 2, 15, bold, dark, 22);
      // Subtle rule below
      page.drawLine({ start: { x: ML, y: y }, end: { x: W - MR, y: y }, thickness: 0.4, color: muted, opacity: 0.25 });
      y -= 10;
    } else if (block.type === "h3") {
      need(40); y -= 8;
      for (const l of wrapText(block.text, bold, 12, contentW)) drawLine(l, ML, 12, bold, accent, 18);
      y -= 4;
