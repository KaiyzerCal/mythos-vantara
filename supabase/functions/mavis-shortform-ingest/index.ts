...
// ── Summary — free-first cascade (Gemini/Groq before Claude Haiku) ────────────
...
async function summariseTranscript(transcript: string, platform: Platform): Promise<string> {
  ...
  try {
    const text = (await callWithFallback(
      "claude",
      [{ role: "user", content: `This is a transcript from a ${platformLabel} video. Summarize the key points in 3-5 bullets, then write a short 2-paragraph summary.\n\nTranscript:\n${transcript.slice(0, 10000)}` }],
      `You are a concise research assistant. Summarize ${platformLabel} video transcripts clearly and helpfully.`,
      PROVIDER_KEYS,
    )).content;
    return text.trim();
  } catch {
