...
// path gets the same one-shot retry instead of surfacing a spurious error
...
export async function withTransientRetry<T extends { error?: unknown } | void>(
  fn: () => Promise<T>,
  retriesLeft = 1,
  delayMs = 800
): Promise<T> {
  try {
    ...
    if (result && (result as { error?: unknown }).error && retriesLeft > 0 && isTransientFetchError((result as { error?: unknown }).error)) {
      ...
      return withTransientRetry(fn, retriesLeft - 1, delayMs);
    }
    return result;
  } catch (e) {
